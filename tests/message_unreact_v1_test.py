...
def test_unreact_invalid_message_id(data):
...
def test_unreact_invalid_react_id(data):
    response = requests.post(url + "message/unreact/v1", json={
        "token": data["token1"], 
...
def test_unreact_invalid_both(data):
    response = requests.post(url + "message/unreact/v1", json={
        "token": data["token1"], 
        "message_id": 93198,
        "react_id": 51218
...
# checking unreact on a user who has already reacted
def test_unreact_valid_react(data):
    response = requests.post(url + "message/unreact/v1", json={
        "token": data["token1"], 
        "message_id": data["message_id"],
        "react_id": 1
...
# checking to see if multiple reacts can be unreacted
def test_unreact_valid_multiple_reacts(data):
    response = requests.post(url + "message/unreact/v1", json={
        "token": data["token1"], 
        "message_id": data["message_id"],
        "react_id": 1
...
# testing when caller of unreact has not reacted to a message
def test_unreact_user_has_not_reacted(data):
