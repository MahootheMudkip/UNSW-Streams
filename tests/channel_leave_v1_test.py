...
def test_invalid_channel(initial_data):
    ...
    resp = requests.get(config.url + 'channel/leave/v1', params={"token": user1_token, "channel_id": 3332})
...
def test_auth_user_not_in_public_channel(initial_data):
    ...
    resp = requests.get(config.url + 'channel/leave/v1', params={"token": user2_token, "channel_id": public_channel_id})
    assert(resp.status_code == ACCESS_ERROR)
    resp2 = requests.get(config.url + 'channel/leave/v1', params={"token": user3_token, "channel_id": public_channel_id})
...
def test_auth_user_not_in_private_channel(initial_data):
    ...
    resp = requests.get(config.url + 'channel/leave/v1', params={"token": user2_token, "channel_id": private_channel_id})
    assert(resp.status_code == ACCESS_ERROR)
    resp2 = requests.get(config.url + 'channel/leave/v1', params={"token": user3_token, "channel_id": private_channel_id})
    assert(resp2.status_code == ACCESS_ERROR)

# test user is invalid and public channel is valid
def test_user_invalid_and_public_channel_valid(initial_data):
    public_channel_id = initial_data["public_channel_id"]
    resp = requests.get(config.url + 'channel/leave/v1', params={"token": 4345, "channel_id": public_channel_id})
    assert(resp.status_code == ACCESS_ERROR)
    resp2 = requests.get(config.url + 'channel/leave/v1', params={"token": 1521, "channel_id": public_channel_id})
    assert(resp2.status_code == ACCESS_ERROR)

# test user is invalid and private channel is valid
def test_user_invalid_and_private_channel_valid(initial_data):
    private_channel_id = initial_data["private_channel_id"]
    resp = requests.get(config.url + 'channel/leave/v1', params={"token": 4345, "channel_id": private_channel_id})
    assert(resp.status_code == ACCESS_ERROR)
    resp2 = requests.get(config.url + 'channel/leave/v1', params={"token": 1531, "channel_id": private_channel_id})
    assert(resp2.status_code == ACCESS_ERROR)
...
def test_user_not_in_channel_and_invalid_channel_id(initial_data):
    user2_token = initial_data["user2_token"]
    resp = requests.get(config.url + 'channel/leave/v1, params={"token": user2_token, "channel_id": 5647})
    assert(resp.status_code == INPUT_ERROR)
    resp2 = requests.get(config.url + 'channel/leave/v1', params={"token": user2_token, "channel_id": 3231})
...
def test_invalid_user_and_channel_id(initial_data):
    resp = requests.get(config.url + 'channel/leave/v1', params={"token": 4332, "channel_id": 1521})
    assert(resp.status_code == ACCESS_ERROR)
    resp2 = requests.get(config.url + 'channel/leave/v1', params={"token": 1542, "channel_id": 5551})
    assert(resp2.status_code == ACCESS_ERROR)
...
# testing users can leave public channel
def test_channel_leave_removes_user_public_channel(initial_data):
    user1_token = initial_data["user1_token"]
    user2_token = initial_data["user2_token"]
    user3_token = initial_data["user3_token"]
