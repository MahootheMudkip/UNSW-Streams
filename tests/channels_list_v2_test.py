...
def test_channels_list_invalid_token():
    response = requests.get(url + "channels/list/v2", params={"token": "hello"})
    assert response.status_code == ACCESS_ERROR
    
#testing when all the channels of a user are private
def test_private_channels(data):
    user_token = data["user2_token"]
    response = requests.get(url + "channels/list/v2", params={"token":user_token})
    assert response.status_code == NO_ERROR

    assert len(response.json()["channels"]) == 2

#testing when all the channels of a user are public
def test_public_channels(data):
    user_token = data["user1_token"]
    response = requests.get(url + "channels/list/v2", params={"token":user_token})
    assert response.status_code == NO_ERROR
...
def test_channels_listl_v1_channels_returned(data):
    user_token1 = data["user1_token"]
    user_token2 = data["user2_token"]
    response1 = requests.get (url + 'channels/list/v2', params={"token": user_token1})
    response2 = requests.get (url + 'channels/list/v2', params={"token": user_token2})
    ...
    assert response1.json()["channels"][1]["name"] == "channel3"
    assert response1.json()["channels"][1]["channel_id"] == data["channel3_id"]
    assert response1.json()["channels"][2]["name"] == "channel5"
    assert response1.json()["channels"][2]["channel_id"] == data["channel5_id"]

    assert response2.json()["channels"][0]["name"] == "channel2"
    assert response2.json()["channels"][0]["channel_id"] == data["channel2_id"]
    assert response2.json()["channels"][1]["name"] == "channel4"
    assert response2.json()["channels"][1]["channel_id"] == data["channel4_id"]