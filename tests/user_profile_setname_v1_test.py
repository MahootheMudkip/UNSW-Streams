...
def test_user_profile_setname_v1_too_short(setup):
    ...
    response = requests.post(URL, json={"token":token,"name_first":"", "name_last":"Shepherd"})
    assert response.status_code == INPUT_ERROR
    # last name too short, first name ok
    response = requests.post(URL, json={"token":token,"name_first":"Karen", "name_last":""})
    assert response.status_code == INPUT_ERROR
    # first name too short, last name too short
    response = requests.post(URL, json={"token":token,"name_first":"", "name_last":""})
    assert response.status_code == INPUT_ERROR
...
def test_user_profile_setname_v1_same_name(setup):
    ...
    response = requests.post(URL, json={"token":token,"name_first":"John", "name_last":"Smith"})
    assert response.status_code == NO_ERROR

    response = requests.get(url + "user/profile/v1", params={"token":token,"u_id":u_id})
    assert response.json()["name_first"] == "John"
    assert response.json()["name_last"] == "Smith"
...
def test_user_profile_setname_v1_different_name(setup):
    ...
    response = requests.post(URL, json={"token":token,"name_first":"Bob", "name_last":"Dylan"})
    assert response.status_code == NO_ERROR

    response = requests.get(url + "user/profile/v1", params={"token":token,"u_id":u_id})
    assert response.json()["name_first"] == "Bob"
    assert response.json()["name_last"] == "Dylan"
